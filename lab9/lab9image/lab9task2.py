...
def rescue(pixels):
    ...
    red = [255, 0, 0]
    black = [0, 0, 0]
    new_pixels = create_uniform_image(len(pixels), len(pixels[0]), [255, 255, 255])
    for i in range(len(pixels)):
        for j in range(len(pixels[0])):
            if pixels[i][j] == gold:
                new_pixels[i][j] = black
            elif pixels[i][j] == maroon:
                new_pixels[i][j] = red
            else:
                new_pixels[i][j] = pixels[i][j]
    return new_pixels
pixels = load_pixels('r.png')
rescued = rescue(pixels)
save_pixels(rescued, 'rescued_rhett.png')
